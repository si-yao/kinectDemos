...
CMainWindow::~CMainWindow()
{
    ...
    // Clean up bitmaps and MATLAB matrices
    if (m_hColorBitmap)
    {
    ...
    if (m_pColorMat)
    {
        mxDestroyArray(m_pColorMat);
    }

    if (m_pDepthMat)
    {
        mxDestroyArray(m_pDepthMat);
    }
...
int CMainWindow::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    HRESULT hr = CreateMainWindow(hInstance);
    if (FAILED(hr))
    {
        return hr;
    }
            ...
            case IDM_DEPTH_FILTER_CANNYEDGE:
                {
                    ...
                    m_matlabHelper.SetDepthFilter(wmID);
                }
                break;
            default:
...
DWORD WINAPI CMainWindow::ProcessThread()
{
    ...
    // Tell the user we are starting the MATLAB engine
    SetStatusMessage(IDS_STATUS_STARTING_MATLAB_ENGINE);

    // Start up the MATLAB engine
    HRESULT hr = m_matlabHelper.InitMatlabEngine();
    if (FAILED(hr)) {
        SetStatusMessage(IDS_ERROR_MATLAB_ENGINE);

        // Tell the window to paint new status
    ...
    while (continueProcessing)
    {
        ...
        if (m_frameHelper.IsInitialized()) 
        {
            ...
            {
                HRESULT hr = m_frameHelper.GetColorImage(m_pColorMat);
                if (FAILED(hr))
                {
                    continue;
                }
                ...
                hr = m_matlabHelper.ApplyColorFilter(m_pColorMat);
                if (FAILED(hr))
                {
                    continue;
                }
                ...
                UpdateBitmap(m_pColorMat, &m_hColorBitmap, &m_bmiColor);
            ...
            if (!m_bIsDepthPaused && SUCCEEDED(m_frameHelper.UpdateDepthFrame())) 
            {
                ...
                UpdateBitmap(m_pDepthMat, &m_hDepthBitmap, &m_bmiDepth);
...
HRESULT CMainWindow::CreateMainWindow(HINSTANCE hInstance)
{
    ...
    LoadString(hInstance, IDS_KINECTMATLAB_WINDOW_CLASS, szWindowClass, _countof(szWindowClass));
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_KINECTMATLAB));
    wcex.hCursor = LoadCursor(NULL, IDC_ARROW);
    wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName = MAKEINTRESOURCE(IDC_KINECTMATLAB);
...
/// <summary>
/// Initializes the color bitmap and color MATLAB matrix
/// </summary>
/// <returns>S_OK if successful, E_FAIL otherwise</returns>
HRESULT CMainWindow::CreateColorImage()
{
    ...
    mwSize dimensions[] = {height, width, m_frameHelper.NUM_RGB_VALUES_PER_PIXEL};
    m_pColorMat = mxCreateNumericArray(m_frameHelper.MATLAB_RGB_MATRIX_NUM_DIMENSIONS, dimensions, mxUINT8_CLASS, mxREAL);

    // Create the bitmap
    WaitForSingleObject(m_hColorBitmapMutex, INFINITE);
    HRESULT hr = CreateBitmap(width, height, &m_hColorBitmap, &m_bmiColor, m_pColorBitmapBits, IDS_ERROR_BITMAP_COLOR);
    ReleaseMutex(m_hColorBitmapMutex);
...
/// <summary>
/// Initializes the depth bitmap and depth MATLAB matrix
/// </summary>
/// <returns>S_OK if successful, E_FAIL otherwise</returns>
HRESULT CMainWindow::CreateDepthImage()
{
    ...
    // Create MATLAB matrix
    mwSize dimensions[] = {height, width, m_frameHelper.NUM_RGB_VALUES_PER_PIXEL};
    m_pDepthMat = mxCreateNumericArray(m_frameHelper.MATLAB_RGB_MATRIX_NUM_DIMENSIONS, dimensions, mxUINT8_CLASS, mxREAL);

    // Create the bitmap
    WaitForSingleObject(m_hDepthBitmapMutex, INFINITE);
    HRESULT hr = CreateBitmap(width, height, &m_hDepthBitmap, &m_bmiDepth, m_pDepthBitmapBits, IDS_ERROR_BITMAP_DEPTH);	
    ReleaseMutex(m_hDepthBitmapMutex);
...
HRESULT CMainWindow::CreateBitmap(DWORD width, DWORD height, HBITMAP* phBitmap, BITMAPINFO* pBmi, void* pBitmapBits, UINT nID)
{
    // Initialize device context and store in global variable
    if (!m_hdc) 
    {
    ...
    pBmi->bmiHeader.biHeight = -static_cast<long>(height);
...
void CMainWindow::UpdateBitmap(mxArray* pImg, HBITMAP* phBitmap, BITMAPINFO* pBmi)
{
    void* pBitmapBits = NULL;
    int height = -pBmi->bmiHeader.biHeight;

    // Convert MATLAB RGB matrix to bitmap
    HRESULT hr = m_matlabHelper.ConvertRgbMxArrayToBitmap(pImg, &pBitmapBits, pBmi);

    if (SUCCEEDED(hr)) {
        // Update bitmap
        SetDIBits(m_hdc, *phBitmap, 0, height, pBitmapBits, pBmi, DIB_RGB_COLORS);
    }
